...
hosp_table_flx <- hosp_table %>% 
  ...
  bg(j = 6, i = ~pct_death >= 60, part = "body", bg = "red") %>% 
...
# Epi curve for all districts

```{r, results = "asis", include = TRUE, echo = FALSE}

# we create a vector of the districts in our surv data 
districts <- unique(surv$district) %>% na.omit()

for(d in districts){
  dat <- surv %>% filter(district == d)   # we filter for one specific within the for loop 
  
  # we create weekly breaks for one specific district
  weekly_breaks <- seq.Date(
      from = floor_date(min(dat$date_onset, na.rm=T), "week", week_start = 1), 
      to   = ceiling_date(max(dat$date_onset, na.rm=T), "week", week_start = 1), 
      by   = "week")    
  
  # we create the plot for one specific district 
  
 p <- # we create the plot for one specific district 
  p <- ggplot(data = dat) +        # set data
    geom_histogram(
    ...
    theme(                           # other theme adjustments
      ...
      axis.title = element_text(     # axis adjustments
        face = "bold"))+             # make bold   
    labs(                            # plot labels
      title    = str_glue("Weekly case incidence for {d} district"),
      subtitle = "",
      x        = "Week of symptom onset",
      y        = "Weekly cases reported",
      caption  = stringr::str_glue(
        "n = {n_cases} from {d}; Shown in Monday weeks by symptom onset.\nOnset dates range from {min_date} to {max_date}\n{n_missing} cases missing date of onset and not shown",
        n_cases = nrow(dat),
        min_date = format(min(dat$date_onset, na.rm=T), format = '%a %d %b %Y'),
        max_date = format(max(dat$date_onset, na.rm=T), format = '%a %d %b %Y'),
        n_missing = fmt_count(dat, is.na(date_onset)))
    )
  
 # we specify the headings 
